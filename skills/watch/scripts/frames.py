...
def get_metadata(video_path: str) -> dict:
    ...
    if ffprobe is None:
        raise SystemExit("ffprobe is not installed. Install with: winget install Gyan.FFmpeg")
    ...
    result = subprocess.run(
        [
            ...
            "-show_format",
            "-show_streams",
            str(Path(video_path).resolve()).replace("\\", "/"),
        ],
        capture_output=True,
        text=True,
    )
...
def extract(
    ...
    ffmpeg = find_ffmpeg()
    if ffmpeg is None:
        raise SystemExit("ffmpeg is not installed. Install with: winget install Gyan.FFmpeg")

    out_dir.mkdir(parents=True, exist_ok=True)
    for existing in out_dir.glob("frame_*.jpg"):
        existing.unlink()

    # 新版 ffmpeg（≥7.0）在 Windows 上对路径处理较严格：
    # 1. 反斜杠路径 muxer 初始化失败
    # 2. 短文件名（如 ADMINI~1）无法识别输出格式
    # 解法：用 resolve() 拿长名（absolute() 会保留短名），转正斜杠
    output_pattern = str((out_dir / "frame_%04d.jpg").resolve()).replace("\\", "/")
    cmd: list[str] = [
        ffmpeg,
        "-hide_banner",
        "-loglevel", "error",
...
def extract_scene_candidates(
    ...
    # 同上：Windows 路径 + 短文件名 + 新版 ffmpeg 兼容
    output_pattern = str((out_dir / "frame_%04d.jpg").resolve()).replace("\\", "/")
    cmd: list[str] = [
        ffmpeg,
        "-hide_banner",
    ...
    cmd += [
        "-i", str(Path(video_path).resolve()).replace("\\", "/"),
        "-vf", vf,
        # -vsync 在新版 ffmpeg（≥7.0）已弃用，改用 -fps_mode
        "-fps_mode", "vfr",
    ]
