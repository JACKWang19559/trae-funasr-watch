#!/usr/bin/env python3
"""Parse a WebVTT or SRT subtitle file into a clean, timestamped transcript.
...
支持两种字幕格式：
- VTT（YouTube 等）：时间戳用 '.' 分隔毫秒，如 00:00:01.500
- SRT（Bilibili 等）：时间戳用 ',' 分隔毫秒，如 00:00:01,500

时间戳正则 TS_RE 用 [.,] 同时匹配两种分隔符，因此 parse_vtt 可解析两种格式。
"""
...
def parse_vtt(path: str) -> list[dict]:
    """解析 VTT 或 SRT 字幕文件，返回带时间戳的文本段列表。

    函数名保留 parse_vtt 以保持向后兼容，实际同时支持 VTT 和 SRT 格式。
    TS_RE 正则用 [.,] 匹配两种毫秒分隔符，解析逻辑对两种格式通用。

    Args:
        path: 字幕文件路径（.vtt 或 .srt）

    Returns:
        段列表，每段形如 {"start": 0.1, "end": 3.16, "text": "..."}，
        已经过滚动去重处理。
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("usage: transcribe.py <vtt-or-srt-path>", file=sys.stderr)
        raise SystemExit(2)
